...
The Swin Transformer encoder is progressively unfrozen to allow the
pretrained visual features to adapt smoothly to the medical imaging task
...
def train(model_name, model, device, args, model_config,
    ...
    Stage B: Unfrozen encoder fine-tuning with differential LR
    ...
    # Differential learning rates: encoder learns slowly, decoder faster
    optimizer = torch.optim.AdamW([
        {"params": getattr(base_model, backbone_attr).parameters(), "lr": lr_b},
        {"params": base_model.decoder.parameters(), "lr": lr_a * 0.5},
        {"params": base_model.final_conv.parameters(), "lr": lr_a * 0.5},
    ])
